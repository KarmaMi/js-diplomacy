...
class Rule extends BaseRule {
  ...
  getErrorMessageForOrder (map, board, order) {
    const $ = this.helper
    switch (board.state.phase) {
      case $.Movement: return this.movementResolver.getErrorMessageForOrder(map, board, order)
      case $.Retreat:
        break
      case $.Build:
        break
  ...
  defaultOrder (map, board, unit) {
    const $ = this.helper
    switch (board.state.phase) {
      case $.Movement: return this.movementResolver.defaultOrder(map, board, unit)
      case $.Retreat:
        break
      case $.Build:
        break
