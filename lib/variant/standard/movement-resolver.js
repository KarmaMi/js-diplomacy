    ...
    orders.forEach(order => {
      ...
      switch (order.type) {
        case 'Hold':
          ...
          const g = gs.get(order.unit.location) || new MovementOrderGroup()
        ...
        case 'Move':
          {
            ...
            const g1 = gs1.get(order.unit.location) || new MovementOrderGroup()
            g1.target = order
            gs1.set(order.unit.location, g1)

            const gs2 = getGroups(order.unit.location.province)
            const x = new MovementOrderGroup()
        ...
        case 'Support':
          {
            ...
            const g1 = gs1.get(order.target.unit.location) || new MovementOrderGroup()
            g1.addRelatedOrder(order)
            gs1.set(order.target.unit.location, g1)

            const gs2 = getGroups(order.unit.location.province)
            const g2 = gs2.get(order.unit.location) || new MovementOrderGroup()
            ...
            if (maxOrders.size === 1) {
              ...
              if (order2.type === 'Move') {
                if (
                  ...
                  updateGroupResult(group, $.Success)
                } else if (order.useConvoy || order2.useConvoy) {
                  updateGroupResult(group, $.Success)
                } else {
                  updateGroupResult(group, $.Bounced)
                }
