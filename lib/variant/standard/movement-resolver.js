  ...
  resolve (map, board, orders) {
    ...
    while (graph.nodes.size !== 0) {
      ...
      groups.forEach(elem => {
        ...
        // b. Calculate power for each order
        let maxPower = 0
        let maxOrders = []
        gs.forEach(group => {
            ...
            switch (order.type) {
              ...
              case 'Move':
                if (!isDefense) {
                  ...
                  if (isSo) {
                    orderResult.set(order, $.Standoff)
                    break
                  }

                  if (
                    [...maxOrders].every(order2 => {
                      if (!order2.destination) return false
                      const result = orderResult.get(order2)
                      return order.type === 'Move' && result !== $.Fail &&
                  ...
                  orderResult.set(order, $.Fail)
                }
                break
              case 'Support':
                orderResult.set(order, (maxOrders.size === 1) ? $.Success : $.Cut)
                break
              case 'Convoy':
                orderResult.set(order, $.Hold)
                break
            }
          } else {
            if (isDefense) {
