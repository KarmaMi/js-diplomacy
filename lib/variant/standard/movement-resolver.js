...
module.exports = class MovementResolver extends Resolver {
  ...
  resolve (map, board, orders) {
    ...
    const nextState =
      ...
      : new State(state.year + 1, $.Autumn, $.Movement)

    const nextUnits = new Map()
    const nextOccupation = new Map([...occupation])
    const nextUnitStatuses = new Map()
    ...
    units.forEach((units, force) => {
      ...
      units.forEach(unit => {
        if (provinceToOrderResult.has(unit.location.province)) {
          ...
          if (result === $.Dislodged) {
            nextUnitStatuses.set(unit, $.Dislodged)
    ...
    const nextBoard =
      new Board(nextState, nextUnits, nextOccupation, nextUnitStatuses, provinceResult)
