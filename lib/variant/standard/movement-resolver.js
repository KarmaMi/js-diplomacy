...
module.exports = class MovementResolver {
  ...
  resolve (map, board, orders) {
    ...
    while (graph.nodes.size !== 0) {
      ...
      let groups = provinces.map(province => {
        const gs = [...provinceToOrderGroups.get(province)] || []
        return [province, gs.filter(g => g[1].target)]
      })

      // 2. Resolve cutting support
      groups.forEach(elem => {
        const groups = elem[1]
        const support = [...groups].find(group => group[1].target.type === 'Support')
        if (!support) return
        if (support[1].target.type !== 'Move') return

        if ([...groups].some(group => {
          return group.target.type === 'Move' &&
            group.target.destination.province !== support.target.destination.province
        })) {
          orderResult.set(support[1].target, $.Cut)
        }
      })
      ...
      groups = groups.map(elem => {
        const [province, gs] = elem

        const gs2 = gs.map(elem => {
          const group = elem[1]
      ...
      groups.forEach(elem => {
        ...
        // a. Check whether move orders can be conducted or not
        const failedMoves = new Set([...gs].filter(group => {
          if (group.target.type !== 'Move') return false
          if (group.target.destination.province !== province) return false
          if (map.canMoveTo(group.target.unit).has(group.target.destination)) return false

          const ps = [...(group.convoy || [])]
            .filter(e => orderResult.get(e) !== $.Dislodged)
        ...
        dislodegedMoves.forEach(group => {
          group.supports.forEach(support => orderResult.set(support, $.Failed))
          if (group.convoy) {
            group.convoy.forEach(support => {
              if (orderResult.get(support) !== $.Dislodged) orderResult.set(support, $.Failed)
            })
          }
        })

        // e. Resolve failed move orders
        failedMoves.forEach(group => {
          orderResult.set(group.target, $.Failed)
          group.supports.forEach(support => orderResult.set(support, $.Failed))
          if (group.convoy) {
            group.convoy.forEach(support => {
              if (orderResult.get(support) !== $.Dislodged) orderResult.set(support, $.Failed)
        ...
        if (defence) {
          ...
          if (maxOrders.has(order)) {
            switch (order.type) {
              ...
              case 'Convoy': break
              case 'Support':
                defence.supports.forEach(support => orderResult.set(support, $.Success))

                if (maxOrders.size === 1) break
            ...
            if (maxOrders.size === 1) {
              ...
              if (defence.power < offensivePower) {
                orderResult.set(order, $.Dislodged)
                if (order.type !== 'Move') {
                  defence.supports.forEach(support => orderResult.set(support, $.Failed))
                }
              } else {
                if (order.type !== 'Move') {
                  orderResult.set(order, (order.type === 'Support') ? $.Cut : $.Success)
                  defence.supports.forEach(support => orderResult.set(support, $.Success))
                }
              }
            } else {
        ...
        offence.forEach(group => {
          ...
          if (orderResult.get(order) === $.Success) {
            group.supports.forEach(support => orderResult.set(support, $.Success))
            if (group.convoy) {
              group.convoy.forEach(support => {
                if (orderResult.get(support) !== $.Dislodged) orderResult.set(support, $.Success)
              })
            }
          } else {
            group.supports.forEach(support => orderResult.set(support, $.Failed))
