  ...
  resolve (map, board, orders) {
    ...
    while (graph.nodes.size !== 0) {
      ...
      groups.forEach(elem => {
        ...
        // d. Resolve failed move orders
        failedMoves.forEach(group => {
          orderResult.set(group.target, $.Fail)
        })
        ...
        if (defence) {
          ...
          switch (order.type) {
            ...
            case 'Support':
              ...
              // Go to (a) if this support order is not an element of maxOrders
            default:
              if (maxOrders.has(order)) {
                orderResult.set(order, $.Success)
              } else {
