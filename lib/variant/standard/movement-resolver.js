  ...
  resolve (map, board, orders) {
    ...
    while (graph.nodes.size !== 0) {
      ...
      while (groups.length !== 0) {
        ...
        // b. Resolve and exclude failed move orders
        ...
        // c. Resolve and exclude dislodged moves if #provinces <= 2
        offence.forEach(group => {
          if (!defence) return
          if (
            provinces.length <= 2 &&
          ...
          if (maxOrders.has(order)) {
            switch (order.type) {
              ...
              case 'Support':
                ...
                // Check self cutting support
                if ([...offence].some(group => !isSelfCuttingSupport(group, defence))) {
