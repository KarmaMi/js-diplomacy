      ...
      for (const province of provinces) {
        ...
        if (defense) {
          if (maxOrders.has(defense)) {
            switch (defense.type) {
              case 'Move': break
              case 'Support':
                orderResult.set(defense, (maxOrders.size === 1) ? $.Success : $.Cut)
                break
              case 'Hold':
        ...
        offense.forEach(target => {
          if (maxOrders.has(target)) {
            ...
            if (defense && defense.type === 'Move' && !orderResult.get(defense)) {
              if (maxOrders.size === 2 && maxOrders.has(defense)) {
                if (defense.destination.province !== target.unit.location.province) {
                  orderResult.set(target, $.Success)
                  return
                }
              }
