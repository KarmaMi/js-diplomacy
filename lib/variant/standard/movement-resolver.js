...
module.exports = class MovementResolver {
  ...
  resolve (map, board, orders) {
    ...
    orders.forEach(order => {
      ...
      switch (order.type) {
        case 'Hold':
          ...
          const g = gs.get(order.unit.location) || new MovementOrderGroup(ruleKeywords, orderResult)
        ...
        case 'Move':
          {
            ...
            const g1 = gs1.get(order.unit.location) || new MovementOrderGroup(ruleKeywords, orderResult)
            g1.target = order
            gs1.set(order.unit.location, g1)

            const gs2 = getGroups(order.unit.location.province)
            const x = new MovementOrderGroup(ruleKeywords, orderResult)
        ...
        case 'Support':
          {
            ...
            const g1 = gs1.get(order.target.unit.location) || new MovementOrderGroup(ruleKeywords, orderResult)
            g1.addRelatedOrder(order)
            gs1.set(order.target.unit.location, g1)

            const gs2 = getGroups(order.unit.location.province)
            const g2 = gs2.get(order.unit.location) || new MovementOrderGroup(ruleKeywords, orderResult)
    ...
    let graph = new OrderDependencyGraph(ruleKeywords, orders, orderResult)
    ...
    while (graph.nodes.size !== 0) {
      ...
      while (groups.length !== 0) {
        ...
        groups.forEach(elem => {
          ...
          const destination = support.target.getDestination()
    ...
    const nextBoard = StandardRuleUtil.getNewBoard(
      board, nextUnits, nextUnitsStatuses, nextProvinceStatuses
    )
